      ...
      if skip_count == skip_step or (face_flag == 1 and face_continue_cut < 5):
          ...
          if len(faceRects) == 1:          #只检测到一张人脸
            ...
            for faceRect in faceRects:  #单独框出每一张人脸
                ...
                cv2.imwrite('weibo_img' + '/' + title + '/' + '{}.jpg'.format(out_count + 1), frame)  # 存储到指定目录
                cv2.imencode('.jpg', frame)[1].tofile('weibo_img' + '/' + title + '/' + '{}.jpg'.format(out_count + 1))
...
def delete_image(path):
    ...
    for file in paths:
        ...
        try:
            ...
            if abs(yaw) > 15 or abs(pitch) > 15 or blur > 0.7 or illumination < 80:
                os.remove(file)
