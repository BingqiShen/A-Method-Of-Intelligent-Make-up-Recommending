...
file_path = 'eye_taohua'
file_lists = os.listdir(file_path)


eye_index = np.empty(shape=[0,4])
...
for file_list in file_lists:
    ...
    #
    # x, y = img.shape[0:2]
    # image_adjust = cv2.resize(cv_bgr_image, int(y / eye_distance * 327.3207535194756),
    #                           int(x / eye_distance * 327.3207535194756))
    #
...
for i in range(len(eye_index)):
    worksheet.write(i,0,eye_index[i,0])
    worksheet.write(i,1,eye_index[i,1])
    worksheet.write(i,2,eye_index[i,2])
    worksheet.write(i,3,eye_index[i,3])
workbook.save('eye_index_taohua.xls')        # 关闭Excel文件
