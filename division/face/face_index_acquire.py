...
file_path = 'face/face picture/face_yuan'
file_lists = os.listdir(file_path)


face_index = np.empty(shape=[0,3])
...
for file_list in file_lists:
    ...
    face_index = np.append(face_index,[[face_shape_index,face_jaw_index,face_jaw_regression_index]],axis=0)
    ...
    # # 表示眼睛倾斜角度
    # eye_arctan = (math.atan(
    #     (int(eye_point4[:, 1]) - int(eye_point1[:, 1])) / (int(eye_point4[:, 0]) - int(eye_point1[:, 0]))) + math.atan(
    #     (int(eye_point7[:, 1]) - int(eye_point10[:, 1])) / (int(eye_point10[:, 0]) - int(eye_point7[:, 0])))) / 2

    ################################################################眉型参数###################################################################
    # eyebrow_point1 = np.array(landmarks[17])
    # eyebrow_point2 = np.array(landmarks[18])
    # eyebrow_point3 = np.array(landmarks[19])
    # eyebrow_point4 = np.array(landmarks[20])
    # eyebrow_point5 = np.array(landmarks[21])
    #
    # eyebrow_point6 = np.array(landmarks[22])
    # eyebrow_point7 = np.array(landmarks[23])
    # eyebrow_point8 = np.array(landmarks[24])
    # eyebrow_point9 = np.array(landmarks[25])
    # eyebrow_point10 = np.array(landmarks[26])
    #画左眉
    ...
    # eyebrow_line_left = np.poly1d(np.polyfit(landmarks_eyebrow_left_x, landmarks_eyebrow_left_y, 5))       #5存疑
    # for t in range(landmarks_eyebrow_left[0,0],landmarks_eyebrow_left[4,0],1):
    #     y_eyebrow_left = np.int(eyebrow_line_left(t))
    #     cv2.circle(cv_bgr_image, (t,y_eyebrow_left),2, color=(139, 0, 0))
    #画右眉
    ...
    # eyebrow_line_right = np.poly1d(np.polyfit(landmarks_eyebrow_right_x, landmarks_eyebrow_right_y, 5))       #5存疑
    # for t in range(landmarks_eyebrow_right[0,0],landmarks_eyebrow_right[4,0],1):
    #     y_eyebrow_right = np.int(eyebrow_line_right(t))
    #     cv2.circle(cv_bgr_image, (t,y_eyebrow_right),2, color=(139, 0, 0))
...
#print(face_index)
print(len(face_index))
####################################################将脸部数据导入至Excel中#####################################################

workbook = xlwt.Workbook(encoding = 'ascii')        # 创建一个工作表
worksheet = workbook.add_sheet('sheet1')            # 创建一个工作表对象
for i in range(len(face_index)):
    worksheet.write(i,0,face_index[i,0])
    worksheet.write(i,1,face_index[i,1])
    worksheet.write(i,2,face_index[i,2])
workbook.save('face_index_yuan.xls')        # 关闭Excel文件
